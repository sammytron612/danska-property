@extends('layouts.app', ['title' => 'Private Landlords','noIndex' => true])
<div style="margin-top:150px" class="container">
    <div class="row row-cols-1 row-cols-lg-2 row-cols-2 gy-5 gx-4">

        <div class="col">
            <h1 class="d-none d-md-block display-3">Keep your tenants happy, reduce your costs.</h1>
            <h3 class="d-block d-md-none">Keep your tenants happy, reduce your costs.</h3>
            <p>Manage work orders, track repairs and hire contractors in just a few clicks. Improve your tenant experience and reduce repair costs with an intuitive dashboard and in-depth reporting.</p>
        </div>
        <div>
            <img data-aos="fade-left" alt="Couple" data-aos-delay="300" class="rounded img-fluid" src="/assets/img/couple.webp">
        </div>
        <div class="col">
            <img data-aos="zoom-out" alt="Repair" data-aos-delay="300" class="img-fluid" src="/assets/img/repair01.webp" alt="sml_prod_core_repair_diag_01" class="show-tablet   image-left-break">
        </div>

        <div class="col">
            <div class="heading">
                <h3>
                    Get maintenance work done, quickly<span class="primary-green">.</span>
                </h3>
            </div>

            <div class="mt-3 copy">
                <p><span> Our Software is an end-to-end maintenance solution designed with both landlords and tenants in mind. </span></p>
                <p><span>Give your tenants the tools they need to raise repairs, and hire the right contractor for the right job - all in a single platform. </span></p>
                <p><span>Improve your time to solve any maintenance issue, and keep your tenants happy. </span></p>
            </div>
        </div>

        <div class="col mb-5">
            <h3 style="">Bring your costs down</h3>
            <p>Take control of your bottom-line with a streamlined repairs process, and get the best value-for-money by accessing a marketplace of verified contractors.</p>
        </div>
        <div class="col">
            <img data-aos="fade-left" alt="Repair" class="rounded img-fluid" src="/assets/img/repairs.webp">
        </div>
    </div>

</div>
